...
"""
This repository contains the Questron system, an AI-driven interview tool that automates candidate assessment by generating 
questions based on resume data, converting them to audio, and evaluating the user's responses.

## Files in this Repository:

1. `interview_bot_research.ipynb`:
    - This Jupyter Notebook contains the core implementation of the Questron interview system.
    - It handles resume data extraction, question generation using a fine-tuned LLM model, and comparison of user responses.
    - The code utilizes libraries such as NLP models for question generation and semantic similarity for response evaluation.

2. `resume.pdf`:
    - A sample resume document in PDF format.
    - This file is used as an example input for the Questron system to extract relevant data such as skills, experience, and education.

3. `skills_interview_results.csv`:
    - A CSV file that stores the results of the interview process.
    - It contains the interview questions generated by the AI model, the ideal answers, and the user's responses.
    - The file also includes a similarity score that evaluates how closely the user's answers match the model's expected answers.

The combination of these files provides a comprehensive system for automating interviews, generating dynamic questions, transcribing answers, and evaluating candidate responses with semantic similarity.
"""
